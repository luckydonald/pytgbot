...
# sync imports
import requests
from requests.exceptions import RequestException as GenericRequestException
...
class SyncBot(BotBase):
    def get_updates(self, offset=None, limit=100, poll_timeout=0, allowed_updates=None, request_timeout=None, delta=timedelta(milliseconds=100), error_as_empty=False):
        ...
        If `error_as_empty` is set to `False` however, all `requests.RequestException` and `TgApiException` exceptions are normally raised.
        ...
        :param error_as_empty: If errors occur which subclasses `requests.RequestException` or `TgApiException`, they  will be logged but not raised.
